...
void reset_map_chara(SDL_Renderer* renderer, character *js, character  *hero,  rect_struct map_rect[][14], new_texture * perso, new_texture * monstre,new_texture * game_over)
{
    ...
    dessine_grille(renderer, WINDOW_WIDTH, WINDOW_HEIGHT);
    SDL_RenderCopy(renderer, monstre[BAS].texture, NULL, &js->pos);
    //blood(js, blood_item,renderer,1 );
    //blood(js, blood_item,renderer,0 );
    //SDL_RenderCopy(renderer, js_life->texture, NULL, &js_life->pos);
    //SDL_RenderCopy(renderer, hero_life->texture, NULL, &hero_life->pos);
    SDL_RenderCopy(renderer, perso[BAS].texture, NULL, &hero->pos);
    SDL_RenderPresent(renderer);
    SDL_Delay(250);
}
...
int acceuil(SDL_Window * window, SDL_Surface * icon, SDL_Surface * menu, SDL_Texture * texture, SDL_Renderer * renderer,SDL_Rect * positionMenu, SDL_Rect * positionClic, SDL_Rect * positionText,Mix_Music * wave,SDL_Event event)
{
    ...
    if( !menu ){
        printf("L'image n'a pas pu être initialisé: %s\n", SDL_GetError());
    }
    SDL_GetWindowBrightness(window); //récupère la luminosité de notre écran

    //Create the texture for the image for the menu
    ...
    //Audio
    wave = Mix_LoadMUS(MUSIC_MENU_PATH);
        ...
        switch( event.type ) {
            ...
            case SDL_MOUSEBUTTONDOWN: //clic gauche
                if ( event.button.button == SDL_BUTTON_LEFT) {
                    positionClic->x = event.button.x; //retrieving the position of the mouse pointer
                    positionClic->y = event.button.y;
                    
                    if ( event.button.x > 97 && event.button.x < 376 && event.button.y > 102 && event.button.y < 230 ) {
                    ...
                    else if ( event.button.x > 97 && event.button.x < 376 && event.button.y > 430 && event.button.y < 558 ) {
